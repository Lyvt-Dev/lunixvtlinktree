...
function startDynamicStars(){
  ...
  let active = 0;
  const MAX = 240; // cap to prevent DOM bloat
  ...
  const spawn = () => {
    ...
    const cleanup = () => { star.removeEventListener('animationend', cleanup); star.remove(); active--; };
    star.addEventListener('animationend', cleanup);
  };

  // spawn loop (~8-9 stars per second)
  const RATE_MS = 120;
  const id = setInterval(spawn, RATE_MS);

  // Stop spawning when page is hidden
  const onHide = () => { clearInterval(id); document.removeEventListener('visibilitychange', onHide); };
